...
ast_new_node(tre_mem_t mem, tre_ast_type_t type, size_t size)
{
  ...
  node = (tre_ast_node_t *)tre_mem_alloc(mem, sizeof(*node));
  if (node == NULL)
    return NULL;
...
set_empty(tre_mem_t mem)
{
  ...
  new_set =( tre_pos_and_tags_t *) tre_mem_alloc(mem, sizeof(*new_set));
  if (new_set == NULL)
    return NULL;
  ...
  new_set[0].o_class = (tre_ctype_t)0;
...
set_one(tre_mem_t mem, int position, int code_min, int code_max,
	tre_ctype_t o_class, tre_ctype_t *neg_classes, int backref)
{
  tre_pos_and_tags_t *new_set;

  new_set = ( tre_pos_and_tags_t *)tre_mem_alloc(mem, sizeof(*new_set) * 2);
  if (new_set == NULL)
    return NULL;
  ...
  new_set[1].o_class = (tre_ctype_t)0;
	  ...
	  int *tags, int assertions)
{
  ...
  new_set =( tre_pos_and_tags_t *) tre_mem_alloc(mem, sizeof(*new_set) * (s1 + s2 + 1));
  if (new_set == NULL)
    return NULL;
  ...
  for (s1 = 0; set1[s1].position >= 0; s1++)
    {
      ...
      new_set[s1].o_class = set1[s1].o_class;
  ...
  for (s2 = 0; set2[s2].position >= 0; s2++)
    {
      ...
      new_set[s1 + s2].o_class = set2[s2].o_class;
	...
	while (p2->position >= 0)
	  {
	    ...
	    trans->assertions = p1->assertions | p2->assertions
	      | (p1->o_class ? ASSERT_CHAR_CLASS : 0)
	     ...
	     const tre_char_t *regex_end)
{
  ...
  for (i = 0; macros[i] != NULL; i += 2)
    {
      if (tre_strlen((const char*)macros[i]) > len)
	continue;
#ifdef TRE_WCHAR
      if (wcsncmp(macros[i], regex, tre_strlen(macros[i])) == 0)
#else /* !TRE_WCHAR */
      if (strncmp((const char*)macros[i],(const char*) regex, tre_strlen((const char*)macros[i])) == 0)
#endif /* TRE_WCHAR */
	{
...
#else /* !TRE_WCHAR */
static const tre_char_t *perl_macros[] =
  { (tre_char_t*)"t",(tre_char_t*) "\t", (tre_char_t*) "n",(tre_char_t*) "\n",(tre_char_t*)  "r",(tre_char_t*) "\r",
    (tre_char_t*)"f",(tre_char_t*) "\f", (tre_char_t*) "a",(tre_char_t*) "\a",(tre_char_t*)  "e",(tre_char_t*) "\033",
    (tre_char_t*)"w", (tre_char_t*)"[[:alnum:]_]", (tre_char_t*)  "W", (tre_char_t*)"[^[:alnum:]_]",(tre_char_t*) "s", (tre_char_t*)"[[:space:]]",
    (tre_char_t*)"S",(tre_char_t*) "[^[:space:]]",(tre_char_t*) "d",(tre_char_t*) "[[:digit:]]", (tre_char_t*) "D", (tre_char_t*)"[^[:digit:]]",
   NULL };
#endif /* !TRE_WCHAR */
...
static reg_errcode_t
expand_ctype(tre_mem_t mem, tre_ctype_t o_class, tre_ast_node_t ***items,
	     int *i, int *max_i, int cflags)
{
  ...
  for (j = 0; (j < 256) && (status == REG_OK); j++)
    {
      c = j;
      if (tre_isctype(c, o_class)
	  || ((cflags & REG_ICASE)
	      && (tre_isctype(tre_tolower(c), o_class)
		  || tre_isctype(tre_toupper(c), o_class))))
	{
	  if (min < 0)
	    min = c;
	  max = c;
		    ...
		    int *num_items, int *items_size)
{
  ...
  reg_errcode_t status = REG_OK;
  tre_ctype_t o_class = (tre_ctype_t)0;
  int i = *num_items;
      ...
      else
	{
	  ...
	  if (re_end - regex >= 3
	      ...
	      if (endptr != re_end)
		{
...
#else /* !TRE_WCHAR */
		  strncpy((char*)tmp_str,(const char*) regex + 2, len);
#endif /* !TRE_WCHAR */
		  tmp_str[len] = '\0';
		  DPRINT(("  class name: %s\n", tmp_str));
		  o_class = tre_ctype(tmp_str);
		  if (!o_class)
		    status = REG_ECTYPE;
		  ...
		  if (status == REG_OK && TRE_MB_CUR_MAX == 1)
		    {
		      status =
			expand_ctype(mem, o_class, items, &i, &max_i, cflags);
		      o_class = (tre_ctype_t)0;
		      skip = 1;
		    }
		  regex = endptr + 2;
	  ...
	  if (o_class && negate)
	    if (*num_neg_classes >= MAX_NEG_CLASSES)
	      ...
	      neg_classes[(*num_neg_classes)++] = o_class;
	  else if (!skip)
	    {
	      status = new_item(mem, min, max, &i, &max_i, items);
	      if (status != REG_OK)
		break;
	      ((tre_literal_t*)((*items)[i-1])->obj)->o_class = o_class;
	    }

	  /* Add opposite-case counterpoints if REG_ICASE is present.
	     This is broken if there are more than two "same" characters. */
	  if (cflags & REG_ICASE && !o_class && status == REG_OK && !skip)
	    {
	      int cmin, ccurr;
  ...
  for (j = 0; j < i && status == REG_OK; j++)
      ...
      if (l != NULL)
	{
	  ...
	  if (num_neg_classes > 0)
	    {
	      l->neg_classes = (tre_ctype_t*)tre_mem_alloc(mem, (sizeof(l->neg_classes)
						   * (num_neg_classes + 1)));
	...
	case PARSE_POST_CATENATION:
	  {
	    ...
	    tre_ast_node_t *tmp_node = (tre_ast_node_t *)ast_new_catenation(mem, tree, result);
	    if (tmp_node == NULL)
	      return REG_ESPACE;
	    result = tmp_node;
	    break;
		  ...
		  if (r != NULL)
		    {
		      ...
		      status = parse_re(mem, stack, &result, r, tre_strlen((const char*)r),
...
tre_compile(regex_t *preg, const tre_char_t *regex, size_t n, int cflags)
{
  ...
  if (!(cflags & REG_NOSUB))
      ...
      if (tnfa->num_tags > 0)
	{
	  ...
	  marker_offs =(int*) xcalloc(tnfa->num_tags, sizeof(*marker_offs));
	  if (marker_offs == NULL)
	    ERROR_EXIT(REG_ESPACE);
	...
	ERROR_EXIT(REG_ESPACE);
      ...
      submatch_data =(tre_submatch_data_t*)  xcalloc(submatch_id, sizeof(*submatch_data));
  ...
  counts =(int*) xmalloc(sizeof(int) * position);
  if (counts == NULL)
    ERROR_EXIT(REG_ESPACE);

  offs = (int*)xmalloc(sizeof(int) * position);
  if (offs == NULL)
    ERROR_EXIT(REG_ESPACE);
...
regncomp(regex_t *preg, const char *regex, size_t n, int cflags)
{
  ...
  xfree(wregex);
#else /* !TRE_WCHAR */
  ret = tre_compile((regex_t *)preg, (const tre_char_t*)regex, n, cflags);
#endif /* !TRE_WCHAR */
